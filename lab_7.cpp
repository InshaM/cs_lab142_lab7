...
class BiST{
	public:
		...
		void insertHelper(Node * current, int value){
			...
			//Else , then compare the current data with value
			else if(value <  current->data){
				//If the value is less than current data, then move left and call insert
				//if left is NULL,insert it.
				//Make the kid's parent the current.
				if(current->left == NULL)	{
					current->left = new Node(value);
					current->left->parent = current;
				}
				//else, then  move to left and call insert.
		...
		//Just finding if an element exists or not.
